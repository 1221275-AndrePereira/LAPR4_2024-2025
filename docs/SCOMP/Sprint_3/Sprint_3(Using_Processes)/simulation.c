...
int init_drones(Drone *drones) {
    ...
    for (int i = 0; i < MAX_DRONES; i++) {
        ...
        if (pid < 0) {
            ...
            drones[i].id = i;
            drones[i].pid = pid;
            drones[i].write_fd = main_to_drone[1];
            drones[i].read_fd = drone_to_main[0];
...
void run_simulation(Drone *drones, int drone_count, Collision *collisions, int *collision_count, SimulationStatus *status) {
    // Run for a fixed number of time steps or until all drones are inactive
    for (current_time_step = 0; current_time_step < MAX_TIME_STEPS; current_time_step++) {
            ...
            for (int j = 0; j < i; j++) {
                if (drones[j].drone_active_status && compare_positions(drones[i].position, drones[j].position)) {
                    printf("Pairwise collision detected between Drone %d and Drone %d at (%d, %d, %d) at time step %d\n",
                           i+1, j+1, pos.x, pos.y, pos.z, current_time_step+1);

                    collisions[*collision_count].time_step = current_time_step+1;
        ...
        // Early termination if no drones are active
        if (!any_active) {
            break;
        }

        // Check collision threshold for early termination
        if (*collision_count >= MAX_COLLISIONS) {
            printf("Collision threshold exceeded. Terminating simulation.\n");
            for (int i = 0; i < drone_count; i++) {
                if (drones[i].drone_active_status) {
                    kill(drones[i].pid, SIGTERM);
                    drones[i].drone_active_status = false;
